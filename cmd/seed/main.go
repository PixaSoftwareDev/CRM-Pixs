...
//   - 1 system role: admin (all permissions, unrestricted)
...
// seedRoles inserts the single admin role with a fixed UUID.
func seedRoles(ctx context.Context, db *pgxpool.Pool) error {
	_, err := db.Exec(ctx, `
		INSERT INTO roles (id, company_id, name, description, is_system) VALUES
		    ($1, $2, 'admin', 'Acceso total al sistema', true)
		ON CONFLICT (id) DO NOTHING`,
		roleAdmin, seedCompanyID,
	)
	if err != nil {
		return err
	}
	slog.Info("roles seeded", "count", 1)
	return nil
}

// seedRBACMatrix gives admin all permissions, unrestricted.
